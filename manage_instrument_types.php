...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_type'])) {
    ...
    if (empty($new_type)) {
        $errors['new_type'] = 'Instrument type name is required.';
        ...
        $stmt = $mysqli->prepare('INSERT INTO instrument_types (name, category) VALUES (?, "Custom")');
        $stmt->bind_param('s', $new_type);
        if ($stmt->execute()) {
            $success = 'Custom instrument type added successfully!';
        } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_type'])) {
    ...
    $stmt = $mysqli->prepare('DELETE FROM instrument_types WHERE id = ? AND category = "Custom"');
    $stmt->bind_param('i', $type_id);
    if ($stmt->execute()) {
        $success = 'Custom instrument type deleted successfully!';
    } else {
...
// Fetch custom types
$stmt = $mysqli->prepare('SELECT id, name FROM instrument_types WHERE category = "Custom" ORDER BY name');
$stmt->execute();
...
<head>
    ...
    <title>FireSafe - Manage Instrument Types</title>
    <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
</head>
<body class="bg-white min-h-screen p-4">
    <div class="container mx-auto">
        <h2 class="text-3xl font-bold mb-6">Manage Custom Instrument Types</h2>
        <?php if ($success): ?>
        ...
        <div>
            ...
            <?php else: ?>
                <p class="text-gray-600">No custom instrument types added yet.</p>
            <?php endif; ?>
            <button onclick="returnToForm()" class="mt-4 inline-block text-blue-600 hover:underline focus:outline-none">Back to Add Instrument Record</button>
        </div>
    </div>
    <script>
        function returnToForm() {
            window.parent.document.querySelector('iframe').src = 'instrument_inventory_form.php';
        }
    </script>
</body>
</html>
